...
class Trainer(object):
    ...
    def backward(self, model, steps, saved_for_backward):
        ...
        #      dw = \d L / d w_{t+1}
        for (data, _, lr), w, gw in reversed(list(zip(steps, params, gws))):
    ...
    def train(self):
        ...
        for epoch, it, (rdata, rlabel) in self.prefetch_train_loader_iter():
            ...
            if it == 0 and ((ckpt_int >= 0 and epoch % ckpt_int == 0) or epoch == 0):
                ...
                self.save_results(steps=steps, subfolder=f'checkpoints/epoch{epoch:04d}')
                evaluate_steps(state, steps, f'Begin of epoch {epoch}')
            ...
            grad_infos.append(self.backward(tmodels, steps, saved))
            del l, saved
            ...
            if do_log_this_iter:
                loss = losses.item()
                logging.info(f'Epoch: {epoch:4d}\tLoss: {loss:.4f}\tData Time: {data_t:.2f}s\tTrain Time: {t:.2f}s')
                if loss != loss:  # nan
